...
#
#   Output (/dist): dvipdfm.worker.js / .js / .wasm
#
...
emcc -O2 -g0 \
  ...
  -o "$OUT/dvipdfm.js" 2>emlink.out || { echo "final link failed"; tail -60 emlink.out; exit 1; }
[ -s "$OUT/dvipdfm.map" ] || { echo "dvipdfmx link map was not generated"; exit 1; }
if grep -E 'libpplib|pp(doc|dict|array|stream|ref|xref)_' "$OUT/dvipdfm.map"; then
  echo "ERROR: forbidden pplib archive or symbol remains in the dvipdfmx link map" >&2
  exit 1
fi
cp "$GLUE/dvipdfm-worker.js" "$OUT/dvipdfm.worker.js"
cp "$GLUE/kpse-resolve.cjs" "$OUT/kpse-resolve.js"
cp "$GLUE/bundle-mode.js" "$OUT/bundle-mode.js"

echo "=== Output ==="
ls -lh "$OUT"/dvipdfm.* || { echo "no output"; exit 1; }
